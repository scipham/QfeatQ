...
h5open("basicLocSubsysdimbenchmark2.dat", "w") do fid
#syssizeRange = collect(3:15)
subsysdimRange = reverse(collect(2:8))
#syssizeRange = reverse(collect(25:4:45))
errArray = []
numRuns = 6
system_size = 2
...
@showprogress 1 "Running..." for runi in 1:numRuns
    @showprogress 1 "Running Through Subsystem Dimensions..." for subsystem_dim in subsysdimRange
        QfeatQ.generateCompleteRandomObservables(80, system_size, subsystem_dim, "randObs.txt", 2)
