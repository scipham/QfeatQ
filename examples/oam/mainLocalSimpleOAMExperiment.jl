...
function mainExperimentController(system_size, subsystem_dim, number_of_measurements,  observableFile="observables.txt")
    ...
    QfeatQ.state_generation_Controller("pdc-oam", system_size, subsystem_dim, "systemState.txt")
...
h5open("oam2basic_simpleEW.dat", "w") do fid
system_size, subsystem_dim = 4,3

QfeatQ.generateSpinEW(string("./observables_spinEW",subsystem_dim,"d.txt"),system_size,subsystem_dim)
...
for nm in Int.(round.((10 .^(range(1.1,stop=4.1,length=12)))))
    for runi in 1:5
        GC.gc()

        p,t,e = mainExperimentController(system_size, subsystem_dim,nm, string("./observables_spinEW",subsystem_dim,"d.txt")) #"../observables_entanglementWitness",3,"d.txt"
